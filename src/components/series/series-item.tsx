    ...
    <>
      <div className="flex flex-col col-span-1 p-4 shadow-md rounded-lg space-y-3 max-h-min">
        <div className="relative w-full h-4 rounded-full bg-gray-200">
          <div
            className={
              ...
              (data.data.maxEpisode === null ? "hidden" : "")
            }
            style={{
              width:
                (data.data.currentEpisode / data.data.maxEpisode) * 100 + "%",
            }}
          ></div>
        </div>
        <span className="font-bold text-xl">{data.data.title}</span>
        <span className="text-lg mt-auto">
          Episode: <span className="font-bold">{data.data.currentEpisode}</span>{" "}
          / {data.data.maxEpisode === null ? "?" : data.data.maxEpisode}
        </span>
        <span
          className={(data.data.status ? "" : "hidden ") + "text-lg mt-auto"}
        >
          Airing Status:{" "}
          <span className="font-bold text-green-600">{data.data.status}</span>
        </span>
        <span
          className={
            (data.data.dayUpdate != "" && data.data.status === "On-Going"
              ? ""
              : "hidden ") + "text-lg"
          }
        >
          Next episode release on{" "}
          <span className="font-bold">{data.data.dayUpdate}</span>
        </span>
        <div className="flex w-full space-x-2 grow">
          ...
          <Button
            text={
              data.data.currentEpisode == data.data.maxEpisode
                ? "Complete"
                : "Next"
            }
            className={
